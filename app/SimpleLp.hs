...
eval (Add expr1 expr2) lut = safeEval (+) expr1 (const True) expr2 (const True) lut
eval (Sub expr1 expr2) lut = safeEval (-) expr1 (const True) expr2 (const True) lut
eval (Mul expr1 expr2) lut = safeEval (*) expr1 (const True) expr2 (const True) lut
eval (Div expr1 expr2) lut = safeEval div expr1 (const True) expr2 (/= 0) lut
eval (Mod expr1 expr2) lut = safeEval mod expr1 (const True) expr2 (/= 0) lut
--
...
-- I'll prob want to decouple booleans from ints later! Or maybe not xd
